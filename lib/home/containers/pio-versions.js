...
export default class PioVersions extends React.Component {
  ...
  componentDidMount() {
    utils.getCoreVersion().then(
      ...
      (err) => {
        return utils.notifyError('Failed to retrieve IntoYunIoT Core version', err);
      }
    );
  }
    ...
    return (
      <div className='block versions'>
        <ul className='list-inline'>
          <li>IDE <code>{ utils.getIDEVersion() }</code></li>
          <li>·</li>
          <li>Core <code className='inline-block'>{ this.renderCoreVersion() }</code></li>
        </ul>
      </div>
    );
  }

      ...
      <div>
        <Table
          ...
          <Thead>
            ...
            <Th column='platform'>硬件平台</Th>
            <Th column='frameworks'>软件框架</Th>
            <Th column='debug'>Debug</Th>
            <Th column='mcu'>MCU</Th>
            <Th column='fcpu'>FRQ</Th>
            <Th column='rom'>ROM</Th>
            <Th column='ram'>RAM</Th>
          </Thead>
          { items.map(item => (
              <Tr key={ item.id }>
                <Td column='name' value={ item.name }>
                  <span>
                    <a onClick={ () => atom.clipboard.write(item.id) } title={ `复制到粘贴板: ${item.id}` }><span className='icon icon-clippy'></span></a>
                    <a href={ item.url }>
                      { item.name }
                    </a>
                  </span>

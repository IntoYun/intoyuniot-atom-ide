      ...
      <div className='native-key-bindings' tabIndex='-1'>
        ...
        <dl className='row inset-panel padded'>
          ...
          <dd className='col-xs-10'>
            <a onClick={ () => atom.clipboard.write(this.props.data.name) }><span title='复制到粘贴板' className='icon icon-clippy'></span></a> <span className='inline-block highlight'>{ this.props.data.name }</span>
          </dd>
          ...
          <dd className='col-xs-10 inline-buttons'>
            { this.props.data.platforms.map(item => (
                <button key={ item.name } onClick={ () => this.props.showPlatform(item.name) } className='btn btn-sm icon icon-tag inline-block-tight'>
                  { item.title }
                </button>
              )) }
          </dd>

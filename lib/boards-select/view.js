...
export class BoardsSelectView extends BaseView {
  ...
  setBoards(boards) {
    this.allBoards = boards.slice(0).map(item => Object.assign({}, item));
    this.filterBoardsChoices();
  }
  ...
  filterBoardsChoices() {
    ...
    for (const boardId of sortedKeys) {
      ...
      if (!groups.hasOwnProperty('intorobot')) {
        groups['intorobot'] = document.createElement('optgroup');
        groups['intorobot'].label = 'IntoRobot Boards';
      }
      ...
      if (board.name.startsWith('IntoRobot')) {
        groups['intorobot'].appendChild(option);
      } else {

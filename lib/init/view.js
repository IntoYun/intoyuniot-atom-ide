...
export class InitializeNewProjectView extends BaseView {
  ...
  updateUploadButtonDisabled (board) {
    // const dfuRadio = this.element.querySelector('#dfu');
    // const stlinkRadio = this.element.querySelector('#stlink');
    const radioGroup = this.element.querySelector('.protocol-view');

    if (board.platform === 'ststm32') {
      radioGroup.style.display = 'flex';
      // dfuRadio.checked = true;
      // stlinkRadio.disabled = false;
    } else {
      radioGroup.style.display = 'none';
      // dfuRadio.checked = true;
      // stlinkRadio.disabled = true;
    }
  }

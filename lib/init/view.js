...
export class InitializeNewProjectView extends BaseView {
  ...
  initialize() {
    ...
    // 配置文件内容
    this.ini = Object.values(getIniObject())[0];
  ...
  initializeBoardsSelect() {
    ...
    this.boardsSelect.setSelectedBoard(this.ini.board);
    this.updateUploadButtonDisabled(this.ini);
  ...
  initializeDevicesSelect () {
    ...
    devices.forEach(device => {
      ...
      if (device.port === this.ini.upload_port) {

...
export function handleUncaughtErrors(err) {
  Telemetry.hitException(err, true);
}
...
export function updateOSEnviron() {
  ...
  if (atom.config.get('intoyuniot-ide.useBuiltinPIOCore')) { // Insert bin directory into PATH
  ...
  handleCustomPATH(atom.config.get('intoyuniot-ide.advanced.customPATH'));
...
export function handleShowPlatformIOFiles() {
  ...
  for (const name of ['.intoiotenvs', '.pioenvs', '.piolibdeps', '.clang_complete', '.gcc-flags.json', '.travis.yml', 'project.json', 'size']) {
    if (!ignoredNames.includes(name)) {
      ignoredNames.push(name);
    }
  }
  atom.config.set('core.ignoredNames', ignoredNames);
  atom.config.set('tree-view.hideIgnoredNames', !atom.config.get('intoyuniot-ide.advanced.showPlatformIOFiles'));
}
...
export function notifyLinterDisabledforArduino() {
  ...
  atom.confirm({
    message: 'IntoYunIoT: Smart Code Linter',
...
export function installCommands() {
  if (config.IS_WINDOWS) {
    const winCheckResult = child_process.spawnSync('intoyuniot', ['--version']);
    if (0 !== winCheckResult.status) {
      ...
      if (0 !== addResult.status) {
        atom.notifications.addError('IntoYunIoT: Failed to install IntoYunIoT commands!', {
          detail: addResult.stderr
        });
        console.error('' + addResult.stderr);
      } else {
        atom.notifications.addSuccess(
          'IntoYunIoT: Commands have been successfully installed'
        );
      }
    }
  } else {
    const args = ['-c', 'command -v intoyuniot --version'];
    ...
    if (0 !== checkResult.status) {
      const map = [
        [path.join(config.ENV_BIN_DIR, 'intoyuniot'), '/usr/local/bin/intoyuniot'],
        [path.join(config.ENV_BIN_DIR, 'intoiot'), '/usr/local/bin/intoiot'],
      ];
      try {
        ...
        atom.notifications.addError('IntoYunIoT: Failed to install commands', {
          detail: msg,
          dismissable: true,
        });
      }
    } else {
      atom.notifications.addInfo('IntoYunIoT: Shell Commands installation skipped.', {

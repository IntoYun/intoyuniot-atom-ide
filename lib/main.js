...
import { destroyStore, initStore, serializeStore } from './core/store';
...
import { checkAuthStatusUnlessDisabled } from './account/helpers';
...
class PlatformIOIDEPackage {
  ...
  activate(state) {
    this.projectObservable = new ProjectObserver();

    window.addEventListener('error', maintenance.handleUncaughtErrors);
    maintenance.updateOSEnviron();

    if (state && state.hasOwnProperty('store')) {
      initStore(state.store);
    }

    this.startInstaller()
      .then(() => checkAuthStatusUnlessDisabled())
  ...
  async startInstaller() {
    ...
    if (im.locked()) {
      atom.notifications.addInfo('IntoYunIoT IDE installation has been suspended.', {
        detail: 'Seems like IntoYunIoT IDE Installer is already started in another window.',
        dismissable: true
      });
  ...
  setupCommands() {
    this.subscriptions.add(atom.commands.add('atom-workspace', {
      ...
      'platformio-ide:initialize-set-project': () => init.setting(),
      'platformio-ide:import-arduino-ide-project': () => importArduinoIDEProject(),

      'platformio-ide:maintenance.open-terminal': () => runCmdsInTerminal(['intoyuniot --help']),
      'platformio-ide:maintenance.serial-monitor': () => serialMonitor(),
      'platformio-ide:maintenance.serial-ports': () => runCmdsInTerminal(['intoyuniot device list']),
      'platformio-ide:maintenance.install-commands': () => maintenance.installCommands(),
      'platformio-ide:maintenance.update-platformio': () => runCmdsInTerminal(['intoyuniot update']),
      'platformio-ide:maintenance.upgrade-platformio': () => runCmdsInTerminal(['intoyuniot upgrade']),

      'platformio-ide:piolpus-site': () => utils.openUrl('https://www.intoyun.com'),
      'platformio-ide:help-docs': () => utils.openUrl('http://docs.intoyun.com'),
      'platformio-ide:help-api': () => utils.openUrl('http://docs.intoyun.com/devicedev/software-develop-master/master-sdk-api'),
    ...
    function makeRunTargetCommand(target) {
      return function() {
        ...
        if (!p) {
          atom.notifications.addWarning('当前工程不是IntoYunIoT项目工程', {
            detail: '请确认您当前打开的工程为IntoYunIoT项目工程（例如：工程内包括intoyuniot.ini文件）。'
          });
          return;
        }
        ...
        if (!status) {
          atom.notifications.addError(`IntoYunIoT: 执行命令: ${target}失败`, {
            detail: '请确认"build"包已经安装并已激活。',
          });
        }
      };
    }
    ...
    this.subscriptions.add(atom.workspace.observeTextEditors((editor) => {
      ...
      if ('intoyuniot.ini' === path.basename(editorPath)) {
  ...
  deactivate() {
    ...
    this.projectObservable.destroy();
  }

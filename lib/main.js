  ...
  setupCommands() {
    ...
    function makeRunTargetCommand(target) {
      return function() {
        ...
        if (!p) {
          atom.notifications.addWarning('Can not find active IntoYunIoT project.', {
            detail: 'Make sure that an opened project you are trying to rebuid is ' +
              'a IntoYunIoT project (e.g., contains `intoyuniot.ini`).'
          });
          return;
        }
        ...
        if (!status) {
          atom.notifications.addError(`IntoYunIoT: Failed to run a command: ${target}`, {

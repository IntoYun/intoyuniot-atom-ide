...
export function getBoards() {
  if (!__BOARDS_CACHE) {
    const child = child_process.spawnSync('intoyuniot', config.DEFAULT_PIO_ARGS.concat(['boards', '--json-output']));
    if (0 !== child.status) {
      throw new Error('Failed to get boards');
    }
    __BOARDS_CACHE = JSON.parse(child.stdout);
  }
  return __BOARDS_CACHE.filter(board => board.id.indexOf('intorobot-') > -1);
}

export function getDevices () {
  const child = child_process.spawnSync('intoyuniot', config.DEFAULT_PIO_ARGS.concat(['device', 'list', '--json-output']));
  if (0 !== child.status) {
...
export function spawnPio(args, options = {}) {
  return new Promise((resolve, reject) => {
    let stdout = '',
      stderr = '';
    const child = child_process.spawn('intoyuniot', config.DEFAULT_PIO_ARGS.concat(args), options);

...
export default class LibrarySearchCard extends React.Component {
  ...
  render() {
    return (
      <div className='block list-item-card'>
        <div className='row'>
          <div className='col-xs-10'>
            <h2>{ this.props.item.name }</h2>
            {/* <h2><a onClick={ (e) => this.onDidShow(e, this.props.item.id) }>{ this.props.item.name }</a>by <small>* this.props.item.authornames.join(', ')</small></h2> */}
          </div>
        ...
        <div className='row'>
          ...
          <div className='col-xs-2 text-right card-actions'>
            <button onClick={ (e) => this.onDidInstall(e, this.props.item.id) } className='btn btn btn-primary icon icon-cloud-download'>安装</button>
          </div>
        </div>
      </div>
    );

...
export default class LibraryStorageItem extends React.Component {
  ...
  render() {
    ...
    return (
      <div className='block list-item-card native-key-bindings' tabIndex='-1'>
        <div className='row'>
          <div className='col-xs-9'>
            <h2>{ this.props.item.name }<small>{ authornames.length ? ` by ${authornames.join(', ')}` : '' }</small></h2>
          </div>
            ...
            <div className='btn-group'>
              ...
              { this.props.onUpdate && this.props.actions & LibraryStorage.ACTION_UPDATE ? (
                <button onClick={ (e) => this.props.onUpdate(e) } className='btn btn-primary icon icon-cloud-download'>
                  { this.props.item.versionLatest ? `更新到${this.props.item.versionLatest}` : '更新' }
                </button>
                ) : ('') }
            </div>
          </div>

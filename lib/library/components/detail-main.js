        ...
        <dl className='row inset-panel padded'>
          ...
          <dd className='col-xs-10'>
            { this.props.data.authors.map(item => (
                <div key={ item.name } className='lib-author'>
            ...
            { this.props.data.frameworks.length &&
              <div className='block'>
                ...
                {/* this.props.data.frameworks.map(item => (
                    <a key={ item.name } onClick={ () => this.onDidFrameworkSearch(item.name) }>
                      { item.title }
                    </a>
                  )) */}
              </div> }
            { this.props.data.platforms.length &&
              <div>
                ...
                {/* this.props.data.platforms.map(item => (
                    <a key={ item.name } onClick={ () => this.onDidPlatformSearch(item.name) }>
                      { item.title }
                    </a>
                  )) */}
              </div> }
          ...
          <dd className='col-xs-10 inline-buttons'>
            {/* this.props.data.keywords.map(name => (
                <button key={ name } onClick={ () => this.onDidKeywordSearch(name) } className='btn btn-sm icon icon-tag inline-block-tight'>
                  { name }
                </button>
              )) */}
          </dd>

  ...
  render() {
    ...
    return (
      <div className='lib-search'>
        <h1 className='section-heading icon icon-book'>搜索<span className='badge badge-medium'>{ totalResults > -1 ? totalResults.toLocaleString() : '' }</span></h1>
        ...
        { status === LibrarySearchPage.STATUS.LOADING &&
          <ul className='background-message text-center'>
            <li>
              <span className='loading loading-spinner-small inline-block'></span> 加载中...
            </li>
          </ul> }
...
// Redux
//  http://docs.intoyun.com/intoyuniot/library-manager/user-guide/#intoyuniot-lib-search
function mapStateToProps(state, ownProps) {
  return {
    ...
    searchLibrary: (query, page) => goTo(ownProps.history, '/lib/registry/search', {
      query,
      page
    }),
    showLibrary: idOrManifest => goTo(ownProps.history, '/lib/registry/show', {
      idOrManifest
    })
  };
}

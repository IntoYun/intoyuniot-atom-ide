...
import openAuthModal from '../account/helpers';
  ...
  async settings() {
    ...
    try {
      const content = await new Promise((resolve, reject) => {
        fs.readFile(
          path.join(this.projectDir, 'intoyuniot.ini'),
          'utf-8',
      ...
      utils.notifyError(`Could not parse "intoyuniot.ini" file in ${this.projectDir}`, err);
      return;
    }
...
class RunTarget {
  ...
  get name() {
    let name = `IntoYunIoT ${this._name}`;
